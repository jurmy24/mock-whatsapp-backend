...
class ClassInfo(BaseModel):
    ...
    def format_readable(self) -> str:
        ...
        for subject, grades in self.classes.items():
            subject_name = SubjectName(subject).display_format
            formatted_grades = sorted(
                [GradeLevel(grade).display_format for grade in grades]
            )
...
class User(SQLModel, table=True):
    ...
    state: UserState = Field(default="new", max_length=50)
    onboarding_state: Optional[OnboardingState] = Field(default="new", max_length=50)
    role: Role = Field(default="teacher", max_length=20)
    ...
    def class_name_to_id_map(self) -> Dict[str, int]:
        ...
        return {
            f"{class_.class_.subject_.name.capitalize()} {GradeLevel(class_.class_.grade_level).display_format}": class_.class_.id
...
class Subject(SQLModel, table=True):
    ...
    """ FIELDS """
    id: Optional[int] = Field(default=None, primary_key=True)
    name: SubjectName = Field(max_length=50, nullable=False)
...
class Class(SQLModel, table=True):
    ...
    grade_level: GradeLevel = Field(max_length=10, index=True)
    status: SubjectClassStatus = Field(default="active")
...
class Resource(SQLModel, table=True):
    ...
    type: Optional[ResourceType] = Field(max_length=30)
...
class Chunk(SQLModel, table=True):
    ...
    chunk_type: Optional[ChunkType] = Field(max_length=30, default=None)
    """
